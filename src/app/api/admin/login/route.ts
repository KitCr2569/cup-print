...
export async function POST(request: Request) {
  ...
  let password="";try{const body=await request.json();password=typeof body?.password==="string"?body.password:""}catch{return Response.json({error:"ข้อมูลไม่ถูกต้อง"},{status:400})}
  const expected=process.env.ADMIN_PASSWORD;if(!expected){console.error("ADMIN_PASSWORD is not configured");return Response.json({error:"ระบบ Admin ยังไม่ได้ตั้งค่า"},{status:503})}
  if(!passwordsMatch(password,expected))return Response.json({error:"รหัสผ่านไม่ถูกต้อง"},{status:401});
  try{await createAdminSession();clearRateLimit(clientKey);return Response.json({ok:true})}catch(error){console.error("Admin session creation failed",error);return Response.json({error:"สร้าง Admin session ไม่สำเร็จ"},{status:500})}
}
